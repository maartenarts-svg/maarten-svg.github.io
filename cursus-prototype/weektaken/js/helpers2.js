...
function bouwVoorkennisTabel(container, matrix, aanklikbaar = false) {
  ...
  const tabel = _maakCriteriaTabel(items, aanklikbaar, false);
  container.appendChild(tabel);
}
...
function bouwSuccescriteriaTabel(container, matrix, aanklikbaar = false, waardenArray = [], onChange = null) {
  ...
  const tabel = _maakCriteriaTabel(items, aanklikbaar, true, waardenArray, onChange);
  container.appendChild(tabel);
}
...
function _maakCriteriaTabel(items, aanklikbaar, metLeerplandoel, waardenArray = [], onChange = null) {
  ...
  items.forEach((item, idx) => {
    ...
    // Kolom 1: tekst (+ leerplandoelen indien van toepassing)
    ...
    const lbl = document.createElement('span');
    lbl.classList.add('lbl');
    const plaatsDelen = (item.plaatscode || '').split('.');
    lbl.textContent   = (plaatsDelen[plaatsDelen.length - 1] || idx + 1) + '|';
    ...
    cb.type    = 'checkbox';
    cb.checked = waardenArray[idx] === true;
    cb.disabled = !aanklikbaar;
    cb.classList.add('criteria-checkbox');
    if (aanklikbaar && onChange) {
      cb.addEventListener('change', () => onChange(idx, cb.checked));
    }
    td2.appendChild(cb);
    tr.appendChild(td2);
