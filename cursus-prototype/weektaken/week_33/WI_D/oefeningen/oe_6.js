// ============================================================
//  oefeningen/oe_6.js  —  Vergelijking: 7/4 y = 1/5
// ============================================================

window.__taakOefening = (function () {

  var CORRECT_ANT_L = 'y';
  var OPL_T = 4, OPL_N = 35;
  var MAX_RIJEN = 10;
  ...
  // Tussenstap juist: LHS bevat y (maar is niet enkel y),
  // en beide kanten worden bij substitutie y=4/35 gelijk aan OPL.
  function _tsJuist(l, r) {
    if (!l || !r) return false;
    var lNorm = _norm(l);
    if (!lNorm.includes('y') || lNorm === CORRECT_ANT_L) return false;
    var lv = _evalMetY(l);
    var rv = _evalMetY(r);
    if (!lv || !rv) return false;
    return lv.valEq(rv) && rv.valEq(new Br(OPL_T, OPL_N));
  }
  ...
  function _frac(t, n) {
    return '<span style="display:inline-flex;flex-direction:column;align-items:center;' +
      'vertical-align:middle;line-height:1.2;margin:0 2px;">' +
      '<span style="border-bottom:1.5px solid currentColor;padding:0 3px;">' + t + '</span>' +
      '<span style="padding:0 3px;">' + n + '</span></span>';
  }
  ...
  function render(container, opgeslagenAntwoorden, verbeterd) {
    ...
    var inhoud = maakOefening({ id: 'oe_6', nummer: 6, container });
    ...
    var n    = saved.length;
    var tsJ  = _tsJuist(saved[0].l, saved[0].r);
    ...
    if (verbeterd) {
      ...
      if (oefJ) {
        ...
        uitw.innerHTML =
          '<div>' + _frac(7,4) + '<em>y</em> = ' + _frac(1,5) + '</div>' +
          '<div style="font-family:Arial,sans-serif;font-style:italic;color:var(--tekst,#1a1a1a);">Je moet in het linkerlid de factor ' + _frac(7,4) + ' wegwerken, dus je moet beide leden delen door ' + _frac(7,4) + '.</div>' +
          '<div>' + _frac(7,4) + '<em>y</em> : ' + _frac(7,4) + ' = ' + _frac(1,5) + ' : ' + _frac(7,4) + '</div>' +
          '<div><em>y</em> = ' + _frac(1,5) + ' · ' + _frac(4,7) + '</div>' +
          '<div><em>y</em> = ' + _frac(4,35) + '</div>';
        fb.appendChild(uitw);
      }
  ...
  function _voegRijToe(inhoud, l, r, verbeterd, jL, jR, insertBefore) {
    var lId = 'oe6-' + (++_idTeller);
    var rId = 'oe6-' + (++_idTeller);
