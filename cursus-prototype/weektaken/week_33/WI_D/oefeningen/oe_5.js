// ============================================================
//  oefeningen/oe_5.js  —  Vergelijking: 1/4 + x = 2/3
// ============================================================

window.__taakOefening = (function () {

  var CORRECT_ANT_L = 'x';
  var OPL_T = 5, OPL_N = 12;
  var MAX_RIJEN = 10;
  ...
  // Tussenstap juist: LHS bevat x (maar is niet enkel x),
  // en beide kanten worden bij substitutie x=5/12 gelijk aan OPL.
  function _tsJuist(l, r) {
    if (!l || !r) return false;
    var lNorm = _norm(l);
    if (!lNorm.includes('x') || lNorm === CORRECT_ANT_L) return false;
    var lv = _evalMetX(l);
    var rv = _evalMetX(r);
  ...
  function render(container, opgeslagenAntwoorden, verbeterd) {
    ...
    var inhoud = maakOefening({ id: 'oe_5', nummer: 5, container });
    ...
    if (verbeterd) {
      ...
      if (oefJ) {
        ...
        uitw.innerHTML =
          '<div>' + _frac(1,4) + ' + <em>x</em> = ' + _frac(2,3) + '</div>' +
          '<div style="font-family:Arial,sans-serif;font-style:italic;color:var(--tekst,#1a1a1a);">Je moet in het linkerlid de term ' + _frac(1,4) + ' wegwerken,<br>dus je moet beide leden verminderen met ' + _frac(1,4) + '.</div>' +
          '<div>' + _frac(1,4) + ' + <em>x</em> − ' + _frac(1,4) + ' = ' + _frac(2,3) + ' − ' + _frac(1,4) + '</div>' +
          '<div><em>x</em> = ' + _frac(8,12) + ' − ' + _frac(3,12) + '</div>' +
          '<div><em>x</em> = ' + _frac(5,12) + '</div>';
        fb.appendChild(uitw);
      }
  ...
  function _voegRijToe(inhoud, l, r, verbeterd, jL, jR, insertBefore) {
    var lId = 'oe5-' + (++_idTeller);
    var rId = 'oe5-' + (++_idTeller);
