// ============================================================
//  correctiesleutels/CR12.js  —  Correctiesleutel Hoofdstuk 12
//
//  Exporteert: render(container, leerling, code, callbacks)
//
//  De matrix H12-datamatrix.js wordt dynamisch geladen via
//  een <script>-tag (var H12_matrix beschikbaar als global).
//  Pad: ../hoofdstukken/H12/H12-datamatrix.js
// ============================================================

// ── Constanten ───────────────────────────────────────────────
const HOOFDSTUK   = 'H12';
const TITEL       = 'Omtrek en oppervlakte';
const SLEUTEL_CODE = 'CR12';

const PARAGRAAF_NAMEN = {
  '12.1': 'Problemen oplossen met omtrek en oppervlakte',
  '12.2': 'Figuren opsplitsen',
};
...
// ── Paragraafnummer ophalen (bv. '12.4' → '4') ───────────────
function paragraafNr(par) {
  return par.split('.')[1];
}

// ── Maximale zichtbare paragraaf bepalen ─────────────────────
// totEnMet is een string zoals '12.3'
...
// ── Matrix laden (eenmalig via global var H12_matrix) ─────────
let _matrixGeladen = false;
function laadMatrix() {
  return new Promise((ok, fout) => {
    if (typeof H12_matrix !== 'undefined') { ok(); return; }
    const s = document.createElement('script');
    s.src = '../hoofdstukken/H12/H12-datamatrix.js';
    s.onload  = () => { _matrixGeladen = true; ok(); };
    s.onerror = () => fout(new Error('H12-datamatrix.js kon niet geladen worden'));
...
function _bouwPagina(container, crData, totEnMet, leerling, callbacks) {
  ...
  const paragrafen = [...new Set(H12_matrix.map(o => o.paragraaf))].sort();
...
function _bouwParagraaf(par, crData, leerling, callbacks) {
  const oefeningen = H12_matrix.filter(o => o.paragraaf === par);
...
function _bouwOpmerkingen(crData, zichtbareParagrafen, leerling) {
  ...
  zichtbareParagrafen.forEach(par => {
    H12_matrix.filter(o => o.paragraaf === par).forEach(o => {
