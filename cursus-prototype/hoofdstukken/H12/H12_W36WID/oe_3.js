...
(function () {
  ...
  opl_a_formule.appendChild(htmlSpan('<i>P</i> = 2π<i>r</i>'));

  const opl_a_berek = document.createElement('span');
  opl_a_berek.appendChild(htmlSpan('2 · π · 5 ≈ 31,42'));

  const opl_b_formule = document.createElement('span');
  opl_b_formule.appendChild(htmlSpan('<i>A</i> = π<i>r</i><sup>2</sup>'));

  const opl_b_berek = document.createElement('span');
  opl_b_berek.appendChild(htmlSpan('π · 5<sup>2</sup> ≈ 78,54'));
  ...
  voegTekstToe(inhoud, 'Bereken de omtrek en de oppervlakte. Rond af op 0,01.', ['intro-tekst']);
  ...
  voegRij(maakOpTdDubbelZStrZdub('•', maakBerekenLabel('cm'), '4cm', false, opl_a_berek), true);
  voegRij(maakAntwoordTd('De omtrek is', '31,42 cm.'), true);
  ...
  voegRij(maakOpTdDubbelZStrZdub('•', maakBerekenLabel('cm<sup>2</sup>'), '4cm', false, opl_b_berek), true);
  voegRij(maakAntwoordTd('De oppervlakte is', '78,54 cm<sup>2</sup>.'), true);
...

  {
    ...
    requestAnimationFrame(() => {
      ...
      fetch('afbeeldingen/oe_36D003.svg')
        .then(r => r.text())
        .then(svgTekst => {
