...
(function () {
  ...
  opl_a_formule.appendChild(htmlSpan('<i>P</i> = som van de zijden'));

  const opl_a_berek = document.createElement('span');
  opl_a_berek.appendChild(htmlSpan('13 + 20 + 11 = 44'));

  const opl_b_formule = document.createElement('span');
  opl_b_formule.appendChild(htmlSpan('<i>A</i> = <i>b</i><i>h</i> : 2'));

  const opl_b_berek = document.createElement('span');
  opl_b_berek.appendChild(htmlSpan('11 · 12 : 2 = 66'));
  ...
  voegRij(maakAntwoordTd('De omtrek is', '44 cm.'), true);
  ...
  voegRij(maakAntwoordTd('De oppervlakte is', '66 cm<sup>2</sup>.'), true);
...

  {
    ...
    requestAnimationFrame(() => {
      ...
      fetch('afbeeldingen/oe_36D005.svg')
        .then(r => r.text())
        .then(svgTekst => {
