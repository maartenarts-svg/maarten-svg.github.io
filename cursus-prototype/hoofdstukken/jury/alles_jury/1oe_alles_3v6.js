...
(function () {
  ...
  opgaveA.appendChild(maakOnderstreping(htmlSpan('- 3<i>x</i>'), 'dubbel', '0.18cm', true));
  opgaveA.appendChild(htmlSpan(' '));
  ...
  antwoordA.appendChild(htmlSpan('-12 - 4<i>x</i>'));

  const opgaveB = document.createElement('span');
  opgaveB.appendChild(htmlSpan('-2(-9 - <i>y</i>)'));

  const antwoordB = document.createElement('span');
  antwoordB.appendChild(htmlSpan('18 + 2<i>y</i>'));

  const inhoud = maakOefening({
    id:'oe_alles_3v6', nummer:'3', soort:'test',
    niveaus:[], tools:[],
