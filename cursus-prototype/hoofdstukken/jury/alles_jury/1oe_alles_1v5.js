/* ============================================================
   alles_jury / oe_alles_1v5.js
   soort: test
   ============================================================ */
(function () {

  const a1 = document.createElement('span');
  a1.appendChild(maakBreuk('7','10','opgave'));
  a1.appendChild(htmlSpan(' + '));
  a1.appendChild(maakBreuk('-1','3','opgave'));

  const a2 = document.createElement('span');
  a2.appendChild(htmlSpan('= '));
  a2.appendChild(maakBreuk('7','10','opgave'));
  a2.appendChild(htmlSpan(' - '));
  a2.appendChild(maakBreuk('1','3','opgave'));

  const a3 = document.createElement('span');
  a3.appendChild(htmlSpan('= '));
  a3.appendChild(maakBreuk('21','30','opgave'));
  a3.appendChild(htmlSpan(' - '));
  a3.appendChild(maakBreuk('10','30','opgave'));

  const a4 = document.createElement('span');
  a4.appendChild(htmlSpan('= '));
  a4.appendChild(maakBreuk('21 - 10','\u200A \u200A \u200A 30\u200A \u200A \u200A ','opgave'));

  const a5 = document.createElement('span');
  a5.appendChild(htmlSpan('= '));
  a5.appendChild(maakBreuk('11','30','opgave'));

  const b1 = document.createElement('span');
  b1.appendChild(maakBreuk('-8','3','opgave'));
  b1.appendChild(htmlSpan(' : '));
  b1.appendChild(maakBreuk('5','2','opgave'));

  const b2 = document.createElement('span');
  b2.appendChild(htmlSpan('= '));
  b2.appendChild(maakBreuk('-8','3','opgave'));
  b2.appendChild(htmlSpan(' · '));
  b2.appendChild(maakBreuk('2','5','opgave'));

  const b3 = document.createElement('span');
  b3.appendChild(htmlSpan('= '));
  b3.appendChild(maakBreuk('-8 · 2','3 · 5','opgave'));

  const b4 = document.createElement('span');
  b4.appendChild(htmlSpan('= '));
  b4.appendChild(maakBreuk('-16','15','opgave'));

  const inhoud = maakOefening({
    id:'oe_alles_1v5', nummer:'1', soort:'test',
    niveaus:[], tools:[],
