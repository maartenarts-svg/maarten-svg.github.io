...
(function () {
  ...
  opgaveA.appendChild(maakOnderstreping(htmlSpan('- 10'), 'enkel', '0.1cm', true));
  opgaveA.appendChild(htmlSpan(' '));
  opgaveA.appendChild(maakOnderstreping(htmlSpan('- <i>a</i>'), 'dubbel', '0.18cm', true));
  opgaveA.appendChild(htmlSpan(' '));
  ...
  antwoordA.appendChild(htmlSpan('-13 - 2<i>a</i>'));

  const opgaveB = document.createElement('span');
  opgaveB.appendChild(htmlSpan('-9(-<i>a</i> + 2)'));

  const antwoordB = document.createElement('span');
  antwoordB.appendChild(htmlSpan('9<i>a</i> - 18'));

  const inhoud = maakOefening({
    id:'oe_alles_3v9', nummer:'3', soort:'test',
    niveaus:[], tools:[],
