...
(function () {
  ...
  opgaveA.appendChild(maakOnderstreping(htmlSpan('+ 7'), 'enkel', '0.1cm', true));
  opgaveA.appendChild(htmlSpan(' '));
  opgaveA.appendChild(maakOnderstreping(htmlSpan('- 3<i>a</i>'), 'dubbel', '0.18cm', true));
  opgaveA.appendChild(htmlSpan(' '));
  opgaveA.appendChild(maakOnderstreping(htmlSpan('+ 2'), 'enkel', '0.1cm', true));

  const antwoordA = document.createElement('span');
  antwoordA.appendChild(htmlSpan('9 - 4<i>a</i>'));

  const opgaveB = document.createElement('span');
  opgaveB.appendChild(htmlSpan('-5(4 - 3<i>b</i>)'));

  const antwoordB = document.createElement('span');
  antwoordB.appendChild(htmlSpan('-20 + 15<i>b</i>'));

  const inhoud = maakOefening({
    id:'oe_alles_3v5', nummer:'3', soort:'test',
    niveaus:[], tools:[],
