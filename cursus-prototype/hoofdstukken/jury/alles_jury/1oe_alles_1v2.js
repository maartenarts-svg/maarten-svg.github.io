/* ============================================================
   alles_jury / oe_alles_1v2.js
   soort: test
   ============================================================ */
(function () {

  const a1 = document.createElement('span');
  a1.appendChild(maakBreuk('5','2','opgave'));
  a1.appendChild(htmlSpan(' - '));
  a1.appendChild(maakBreuk('-4','3','opgave'));

  const a2 = document.createElement('span');
  a2.appendChild(htmlSpan('= '));
  a2.appendChild(maakBreuk('5','2','opgave'));
  a2.appendChild(htmlSpan(' + '));
  a2.appendChild(maakBreuk('4','3','opgave'));

  const a3 = document.createElement('span');
  a3.appendChild(htmlSpan('= '));
  a3.appendChild(maakBreuk('15','6','opgave'));
  a3.appendChild(htmlSpan(' + '));
  a3.appendChild(maakBreuk('8','6','opgave'));

  const a4 = document.createElement('span');
  a4.appendChild(htmlSpan('= '));
  a4.appendChild(maakBreuk('15 + 8','\u200A \u200A \u200A 6\u200A \u200A \u200A ','opgave'));

  const a5 = document.createElement('span');
  a5.appendChild(htmlSpan('= '));
  a5.appendChild(maakBreuk('23','6','opgave'));

  const b1 = document.createElement('span');
  b1.appendChild(maakBreuk('-7','4','opgave'));
  b1.appendChild(htmlSpan(' : '));
  b1.appendChild(maakBreuk('-2','3','opgave'));

  const b2 = document.createElement('span');
  b2.appendChild(htmlSpan('= '));
  b2.appendChild(maakBreuk('-7','4','opgave'));
  b2.appendChild(htmlSpan(' · '));
  b2.appendChild(maakBreuk('-3','2','opgave'));

  const b3 = document.createElement('span');
  b3.appendChild(htmlSpan('= '));
  b3.appendChild(maakBreuk('7 · 3','4 · 2','opgave'));

  const b4 = document.createElement('span');
  b4.appendChild(htmlSpan('= '));
  b4.appendChild(maakBreuk('21','8','opgave'));

  const inhoud = maakOefening({
    id:'oe_alles_1v2', nummer:'1', soort:'test',
    niveaus:[], tools:[],
