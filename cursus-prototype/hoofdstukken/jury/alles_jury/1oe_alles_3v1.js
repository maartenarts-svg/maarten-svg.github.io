...
(function () {
  ...
  opgaveA.appendChild(maakOnderstreping(htmlSpan('+ 6'), 'enkel', '0.1cm', true));
  opgaveA.appendChild(htmlSpan(' '));
  opgaveA.appendChild(maakOnderstreping(htmlSpan('- <i>a</i>'), 'dubbel', '0.18cm', true));
  opgaveA.appendChild(htmlSpan(' '));
  opgaveA.appendChild(maakOnderstreping(htmlSpan('- 9'), 'enkel', '0.1cm', true));

  const antwoordA = document.createElement('span');
  antwoordA.appendChild(htmlSpan('-3 + 2<i>a</i>'));

  const opgaveB = document.createElement('span');
  opgaveB.appendChild(htmlSpan('-2(-5 + 3<i>b</i>)'));

  const antwoordB = document.createElement('span');
  antwoordB.appendChild(htmlSpan('10 - 6<i>b</i>'));

  const inhoud = maakOefening({
    id:'oe_alles_3v1', nummer:'3', soort:'test',
    niveaus:[], tools:[],
