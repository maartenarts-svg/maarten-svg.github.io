...
(function () {
  ...
  opgaveA.appendChild(htmlSpan(' '));
  opgaveA.appendChild(maakOnderstreping(htmlSpan('- 2<i>x</i>'), 'dubbel', '0.18cm', true));

  const antwoordA = document.createElement('span');
  antwoordA.appendChild(htmlSpan('-9 - 3<i>x</i>'));

  const opgaveB = document.createElement('span');
  opgaveB.appendChild(htmlSpan('-7(-3<i>y</i> - 1)'));

  const antwoordB = document.createElement('span');
  antwoordB.appendChild(htmlSpan('21<i>y</i> + 7'));

  const inhoud = maakOefening({
    id:'oe_alles_3v10', nummer:'3', soort:'test',
    niveaus:[], tools:[],
