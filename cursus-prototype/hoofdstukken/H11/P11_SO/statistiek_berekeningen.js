...
/* ------------------------------------------------------------
   ...
   - middenLinks:  getal op positie 9 (0-gebaseerd)
   - middenRechts: getal op positie 10 (0-gebaseerd)
   - berekening:   "(g[9] + g[10]) : 2"
...
function berekenMediaan(data) {
  ...
  const middenLinks  = gesorteerd[9];
  const middenRechts = gesorteerd[10];
  const mediaan = Math.round(((middenLinks + middenRechts) / 2) * 10) / 10;
  return {
    gesorteerd,
    reeks,
    middenLinks,
